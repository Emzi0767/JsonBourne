...
# Rebuild-docs
#
# Rebuilds the documentation for Clyde.NET project, and places artifacts in specified directory.
# 
# Author:       Emzi0767
# Version:      2019-01-21T17:39+02:00
#
# Arguments:
#   .\build\rebuild-docs.ps1 <path to docfx> <output path> <docs project path>
#
# Run as:
#   .\build\rebuild-docs.ps1 .\path\to\docfx\project .\path\to\output project-docs
...
function Install-7zip([string] $target_dir_path)
{
    ...
    try
    {
        Write-Host "Extracting 7-zip 9.20 CLI"
...
function Package-Docs([string] $target_dir_path, [string] $output_dir_path, [string] $pack_name)
{
    ...
    # Check if pre-packaging was a success
    if ($LastExitCode -ne 0)
    {
        Return $LastExitCode
    }
